...
int main(int argc, char *argv[]) {
    if (argc != 2) {
        printf("Usage: ./serveur1-LaPaireDeSockets <port>\n");
        exit(1);
    }
    ...
    int s, sock_data, i, j, sock_len = sizeof(si_other), recv_len, packet_size, data_size;
    int seq = 1;
    FILE* file = NULL;
    int fichier_fini = 0;
    int cwnd = SLOW_INIT;
    int cwnd_futur = SLOW_INIT;
    int num_ack = 0, ack_actuel = 0;

    // Timeout en réception des paquets
    struct timeval tv;
    tv.tv_sec = 0; // Temps en secondes
    tv.tv_usec = 100000; // Temps en microsecondes
    ...
    while(!fichier_fini) {
        ...
        // Envoi de la rafale de la fenêtre
        for (j = 0; j < cwnd; j++) {
