...
nbpasswords = 100000000 # would be larger in real-life
...
def cracksha(database):
    ...
    passwords = [''.join(combo) for combo in product(ascii_letters+digits, repeat=5)]

    hashed_passwords = genshahashes(passwords)
    crackeddb = []
    for i in database:
        pwd = getpassfromshahash(hashed_passwords, i[1])
        crackeddb.append((i[0],pwd)) # second argument should contain cleartext password
    return crackeddb
